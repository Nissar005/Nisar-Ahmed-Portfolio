      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 relative grid lg:grid-cols-[1.1fr_0.9fr] gap-16 lg:gap-24 items-center">
        <div>
          ...
          <motion.p initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.18 }} className="mt-5 text-ink-dim text-lg leading-relaxed max-w-xl">
            I build scalable cloud infrastructure, automated CI/CD pipelines, and reliable containerized platforms with a focus on AWS, DevOps, automation, QA, and web development.
          </motion.p>

          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.24 }} className="mt-7 flex flex-wrap gap-2">
            {keywords.map((kw) => (
              <span key={kw} className="font-mono text-xs text-ink-dim border border-line rounded-md px-2.5 py-1 bg-panel/60">{kw}</span>
            ))}
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }} className="mt-9 flex flex-wrap items-center gap-3">
            <a href="#projects" className="inline-flex items-center gap-2 rounded-md bg-mint text-void font-semibold px-5 py-3 hover:bg-mint/90 transition-colors">
              View Projects <ArrowRight size={16} />
            </a>
            <a href="#contact" className="inline-flex items-center gap-2 rounded-md border border-line bg-surface/60 px-5 py-3 text-ink hover:border-mint-dim hover:text-mint transition-colors">
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.46 }} className="mt-10 grid grid-cols-3 max-w-xl border-y border-line-soft py-4">
...
function HeroFlowVisual() {
  return (
    <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7, delay: 0.15 }} className="relative border-l border-line pl-7 sm:pl-10 py-2">
      <div className="font-mono text-xs text-ink-faint mb-7 flex items-center justify-between border-b border-line-soft pb-4">
        ...
        <span className="flex items-center gap-1.5 text-mint"><span className="w-1.5 h-1.5 rounded-full bg-mint animate-pulse" /> live</span>
      </div>
      <div className="relative flex flex-col items-stretch max-w-md">
        {heroFlow.map((node, i) => (
          <div key={node.id} className="relative">
            <motion.div initial={{ opacity: 0, x: -8 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.06 }} className="flex items-center gap-3 py-3">
            ...
            {i < heroFlow.length - 1 && <div className="ml-[34px] w-px h-3 bg-line relative overflow-hidden"><motion.span className="absolute left-0 top-0 w-px h-2 bg-mint" animate={{ y: ["-8px", "16px"] }} transition={{ duration: 1.6, repeat: Infinity, delay: i * 0.2, ease: "linear" }} /></div>}
          </div>
        ))}
      </div>
    </motion.div>
